...
import classes from './ModalForVerifyAll.module.css'
...
const VerifyAll= (props)=>{
  ...
  return(
    <div className={classes.container}>
       ...
       <div>
        <p className={classes.ps}> هل أنت متأكد من  تأكيد كل الحسابات </p>
       </div>
...
export const ModalForVerifyAll = (props) => {
console.log(props)
    return (
        <React.Fragment>
            {/* {ReactDOM.createPortal(<Backdrop close={props.close} />, document.getElementById('modal'))} */}
            {ReactDOM.createPortal(<VerifyAll api={props.api}  close={props.close}/>, document.getElementById('modal'))}
        </React.Fragment>
    );
