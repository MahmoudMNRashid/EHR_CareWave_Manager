...
import classes from './EditTipForm.module.css'
import { useNavigate} from 'react-router-dom';
import { getToken } from '../../Util/Auth';
import { toast } from 'react-toastify';
import LoadingBar from 'react-top-loading-bar';
export const EditTipForm = (props) => {


    const [enteredName, setEnteredName] = useState(props.info ? props.info.titile : '');
    const [enteredNameTouched, setEnteredNameTouched] = useState(false);
    const [progress, setProgress] = useState(0);
    const [enteredDesc, setEnteredDesc] = useState(props.info ? props.info.post : '');
        ...
        <>
            <form className={classes.form} onSubmit={formSubmissionHandler}>
                <input
                    ...
                    value={enteredName}
                // defaultValue={props.info.titile!=='' ? props.info.titile: ''}
                />
                {nameInputIsInvalid && (
                ...
                // defaultValue={props.info.post!=='' ? props.info.post: ''}
